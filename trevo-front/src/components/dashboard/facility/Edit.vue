      ...
      <div class="col-12" enctype="multipart/form-data">
        ...
        <div class="hyper-card mb-3">
          <div class="form-group">
            ...
            <input type="file" class="form-control-file" placeholder="" @change="onLogoFileChange"><br/>
            <button class="btn btn-outline-danger" @click="removeLogo()">remover logo</button>
          </div>
          <div class="form-group">
            <label class="col-form-label" for="formName"><i class="far fa-image" aria-hidden="true"></i> Imagem de Fundo</label>
            <small class="text-muted">escolha uma imagem quadrada</small>
            <input type="file" class="form-control-file" placeholder="" @change="onBackgroundFileChange"><br/>
            <button class="btn btn-outline-danger" @click="removeBackgroundImage()">remover imagem de fundo</button>
          </div>
        </div>
        <div class="hyper-card mb-3">
        ...
        <button class="btn btn-success btn-block" @click="edit()">Salvar alterações</button>
        <br>
        <br>
      </div>
    </div>
...
export default {
  ...
  created () {
    ...
    .then(response => {
      if (response.data.length === 0) {
        ...
        this.facility.address = response.data.address[0]
        ...
        if (response.data.group) {
          this.facility.group = { name: { name: response.data.group.name, _id: response.data.group._id }, label: response.data.group.name }
        }
        if (response.data.department === 'radiodiagnosis') {
          this.facility.department = { name: 'radiodiagnosis', label: 'Radiodiagnóstico' }
        } else if (response.data.department === 'nuclearmedicine') {
          this.facility.department = { name: 'nuclearmedicine', label: 'Medicina Nuclear' }
        } else {
          this.facility.department = { name: 'both', label: 'Ambos' }
        }

        var facilityUsers = response.data.users

        for (var j = 0; j < response.data.users.length; j++) {
          if (response.data.users[j].responsabilities.indexOf('Titular da instalação') >= 0) {
            this.facilityTitular = response.data.users[j].name
          }
          if (response.data.users[j].responsabilities.indexOf('Responsável técnico') >= 0) {
            this.tecnicalReponsible = response.data.users[j].name
          }
          if (response.data.users[j].responsabilities.indexOf('Substituto do responsável técnico') >= 0) {
            this.tecnicalReponsibleSubs = response.data.users[j].name
          }
          if (response.data.users[j].responsabilities.indexOf('Supervisor de radioproteção') >= 0) {
            this.radioprotectionSupervisor = response.data.users[j].name
          }
          if (response.data.users[j].responsabilities.indexOf('Substituto do supervisor de radioproteção') >= 0) {
            this.radioprotectionSupervisorSub = response.data.users[j].name
          }
        }
        ...
        .then(resp => {
          if (resp.data.length === 0) {
            ...
            for (var j = 0; j < resp.data.length; j++) {
              for (var i = 0; i < facilityUsers.length; i++) {
                if (resp.data[j]._id === facilityUsers[i]._id) {
                  flag = true
                  facilityUsers[i].type = resp.data[j].type
                }
              }
              if (flag === false) {
                this.users.push({_id: resp.data[j]._id, name: resp.data[j].name, type: resp.data[j].type, responsabilities: [], ANLicenseNumber: resp.data[j].ANLicenseNumber, FMLicenseNumber: resp.data[j].FMLicenseNumber})
              } else {
                flag = false
              }
            }
            this.facility.users = facilityUsers.sort()
          }
        })
        .catch(error => {
    ...
    .then(resp => {
      if (resp.data.length === 0) {
        ...
        for (let i = 0; i < resp.data.length; i++) {
          this.groupOptions.push({ name: { name: resp.data[i].name, _id: resp.data[i]._id }, label: resp.data[i].name })
        }
  ...
  methods: {
    edit () {
      ...
      auth.editFacility(this, credentials, '/facility')
    },

    removeLogo () {
      if (confirm('tem certeza que deseja remover a logo?')) {
        auth.removeFacilityLogo(this, this.facility._id, () => setTimeout(() => {
    ...
    removeBackgroundImage () {
      if (confirm('tem certeza que deseja remover a imagem de fundo?')) {
        auth.removeFacilityBackgroundImage(this, this.facility._id, () => setTimeout(() => {
