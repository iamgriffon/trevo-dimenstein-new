...
export default {
  ...
  watch: {
    ...
    formatedValidity (val) {
      if (val) {
        ...
        console.log('this.validity', moment(this.validity).format())
      }
    }
  },
  ...
  methods: {
        ...
        if (equipment.facility) {
          if (equipment.facility._id) {
            ...
            .then(response => {
              ...
              let val = facility.address[0].street + ', ' + facility.address[0].number + (facility.address[0].complement ? ', ' + facility.address[0].complement : '') + ', ' + facility.address[0].city + ' - ' + facility.address[0].state
    ...
    async generateTest (test) {
      ...
      // const epiEquipmentAssessment = await auth.registerEPIEquipmentAssessmentTest({
      //   title: test.title,
      //   status: 'Liberado',
      //   date: test.formatedDate,
      //   validity: test.formatedValidity,
      //   serviceOrder: test.serviceOrder,
      //   facilityAddress: this.facilityAddress,
      //   avaliationMode: this.avaliationMode,
      //   equipmentFacilityAddress: this.equipmentFacilityAddress,
      //   epiType: test.epiType,
      //   size: test.size,
      //   sector: test.sector,
      //   identification: test.identification,
      //   color: test.color,
      //   ca: test.ca,
      //   useGoal: test.useGoal,
      //   conclusion: test.avaliation,
      //   uploadedBy: { _id: currentUser.id, name: currentUser.name },
      //   signedBy: { _id: currentUser.id, name: currentUser.name },
      //   liberatedBy: { _id: currentUser.id, name: currentUser.name },
