  ...
  <div class="main">
    <div class="col-12 row m-0 p-0">
      ...
      <div class="col-2 text-right">
        <router-link class="btn btn-success" :to="'/equipments/doseCalibrator/' + equipment._id + '/linearity/register'"><i class="fa fa-plus d-sm-block d-md-none" aria-hidden="true"></i><span class="d-none d-sm-block"><i class="fa fa-plus" aria-hidden="true"></i> adicionar</span></router-link>
      </div>
    </div>
    <div class="col py-0">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb my-0">
          ...
          <li class="breadcrumb-item active" aria-current="page">linearidade</li>
        </ol>
      </nav>
    </div>
    <div class="row">
        ...
        <table class="table table-responsive hyper-table col mx-auto">
          ...
          <tbody >
            <tr v-for="test in tests">
              ...
              <td>{{ test.status }}</td>
              <td>{{ test.conclusion }} <span v-if="test.conclusion != 'satisfatório'" class="red"><i class="fas fa-exclamation-circle"></i></span></td>
              ...
              <td v-else class="text-right"><router-link :to="'/equipments/doseCalibrator/' + equipment._id + '/linearity/' + test._id" class="btn btn-outline-metalic-seaweed"><i class="fa fa-info" aria-hidden="true"></i> informações</router-link></td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  created () {
    ...
    .then(response => {
      if (response.data.length === 0) {
        ...
        axios.get(auth.apiUrl() + 'equipment/' + this.$route.params.id + '/doseCalibratorLinearityTests/', {headers: {Authorization: 'Bearer ' + auth.getToken()}})
        .then(r => {
          if (r.data.length === 0) {
            this.errors = 'Nenhum resultado encontrado'
            this.tests = ''
