<template>
  <div class="main">
    <div>
      <b-modal id="modal-center" centered no-close-on-backdrop no-close-on-esc hide-header hide-header-close hide-footer>
        <p class="col-12 mx-auto inline-block" style="width: fit-content">Fazendo upload do documento...</p>
        <img class="img img-fluid" src="/static/img/loading.gif">
      </b-modal>
    </div>
    <div class="col-12 row m-0 p-0">
      <div class="col">
        <h1>{{ equipment.type + ' ' + equipment.patrimonialNumber }}</h1>
        <h2>Janelas Assimétricas - {{ test.date | moment('MM-YYYY') }}</h2>
        <h3 v-if="test.isAcceptanceTest"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> Teste de Aceite</h3>
      </div>
      <div class="col text-right">
        <button @click="finishTest()" v-if="permission.canExcludeFacility(currentUser) && test.status !== 'Concluído'" class="btn btn-outline-success"><i class="fas fa-check d-sm-block d-md-none" aria-hidden="true"></i><span class="d-none d-md-block"><i class="fas fa-check" aria-hidden="true"></i> finalizar teste</span></button>
        <button @click="exclude()" v-if="permission.canExcludeFacility(currentUser)" class="btn btn-outline-danger"><i class="far fa-trash-alt d-sm-block d-md-none" aria-hidden="true"></i><span class="d-none d-md-block"><i class="far fa-trash-alt" aria-hidden="true"></i> excluir</span></button>
      </div>
    </div>
    <div class="col py-0">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb my-0">
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/panel'"><i class="fa fa-home" aria-hidden="true"></i></router-link></li>
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/facility'"><i class="far fa-building" aria-hidden="true"></i></router-link></li>
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/facility/' + facility._id">{{ facility.name }}</router-link></li>
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/facility/' + facility._id + '/equipments'">equipamentos</router-link></li>
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/facility/' + facility._id + '/equipments/doseCalibrator/'">{{ equipment.type }}</router-link></li>
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/equipments/gamaCamara/' + equipment._id">{{ equipment.patrimonialNumber }}</router-link></li>
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/equipments/gamaCamara/' + equipment._id + '/asymmetricWindow/'">janelas assimétricas</router-link></li>
          <li class="breadcrumb-item active" aria-current="page">{{ test.date | moment('MM-YYYY') }}</li>
        </ol>
      </nav>
    </div>

    <div class="row ml-md-3">
      <div class="col mb-3 pl-sm-1">
        <div class="hyper-card mb-3">
          <h5 class="mb-3">Informações:</h5>
          <p>Data de Realização: {{ test.date | moment('DD/MM/YYYY')  }}</p>
          <p>Matrix Utilizada: {{ test.usedMatrix }}</p>
          <p>Número Total de Contagens: {{ test.totalCountNumber }}</p>
          <p>Taxa de Contagem (Kcont/s): {{ test.countTax }}</p>
          <p>Atividade (µCi) da Fonte Pontual: {{ test.pontualSourceActivity }}</p>
          <p><span v-if="test.uniformityCorrection">Teste realizado com correção de uniformidade</span><span v-else>Teste realizado sem correção de uniformidade</span></p>
          <p>Responsável pela Execução: {{ test.responsibleForPerforming.name  }}</p>
        </div>
      </div>
    </div>

    <div class="row col pr-0" v-if="equipment.numberOfDetectors === '2'">
      <div class="hyper-card col ml-3 mr-0" v-for="i in numbers">
        <h4>Detector {{ i + 1 }}</h4>
        <br>
        <div class="row">
          <div class="col-2">
            Energia
          </div>
          <div class="col-3">
            Uniformidade Integral Calculada
          </div>
          <div class="col-7">
            Análise
          </div>
        </div>
        <br>
        <div v-if="i === 0" v-for="energyAndResult in test.energyAndResultsDetector1">
          <div class="row">
            <div class="col-2">
              {{ energyAndResult.energy }}
            </div>
            <div class="col-3">
              {{ parseFloat(energyAndResult.integralCalculatedUniformity).toFixed(2) }} %
            </div>
            <div class="col-7">
                {{ energyAndResult.dataCheckStatus }}
            </div>
          </div>
          <hr>
        </div>
        <div v-if="i === 1" v-for="energyAndResult in test.energyAndResultsDetector2">
          <div class="row">
            <div class="col-2">
              {{ energyAndResult.energy }}
            </div>
            <div class="col-3">
              <template v-if="energyAndResult.integralCalculatedUniformity">{{ parseFloat(energyAndResult.integralCalculatedUniformity).toFixed(2) }} %</template>
            </div>
            <div class="col-7">
                {{ energyAndResult.dataCheckStatus }}
            </div>
          </div>
          <hr>
        </div>
        <div class="pb-2" v-if="i === 0" ><h5>Observações: </h5>{{ test.energyAndResultsDetector1Obs }}</div>
        <div class="pb-2" v-if="i === 1" ><h5>Observações: </h5>{{ test.energyAndResultsDetector2Obs }}</div>
        <hr>
        <div class="pb-2" v-if="test.results[i]"><h5>Resultado: </h5>{{ test.results[i] }}</div>
        <div><h5 v-if="test.results[i]">Conclusão:</h5> {{ test.conclusions[i] }}</div>
      </div>
    </div>
    <br>
    <div class="row col pr-0" v-if="test.status === 'Concluído'">
      <div class="hyper-card col ml-3 mr-0">
        <p>Data de Finalização: {{ test.finalizingDate | moment('DD/MM/YYYY')  }}</p>
        <p>Responsável por Finalizar: {{ test.responsibleForFinalizing.name }}</p>
      </div>
    </div>
    <br><br>
  </div>
</template>

<script>
import auth from '../../../../../../authentication/authentication'
import permission from '../../../../../../../../services/permissions'
import router from '../../../../../../../routes'
import axios from 'axios'
import moment from 'moment'
import JsPDF from 'jspdf'

export default {
  data () {
    return {
      numbers: [0, 1],
      facility: {
        name: '',
        equipments: []
      },
      test: { responsibleForPerforming: { name: '' } },
      equipment: {},
      currentUser: {},
      permission: permission,
      loading: false
    }
  },

  watch: {
    loading () {
      if (this.loading) {
        this.$root.$emit('bv::show::modal', 'modal-center')
      }
    }
  },

  created () {
    axios.get(auth.apiUrl() + 'equipment/' + this.$route.params.equipmentId, {headers: {Authorization: 'Bearer ' + auth.getToken()}})
    .then(response => {
      if (response.data.length === 0) {
        this.errors = 'Nenhum resultado encontrado'
        this.equipment = ''
      } else {
        this.equipment = response.data

        axios.get(auth.apiUrl() + 'facility/' + response.data.facility._id, {headers: {Authorization: 'Bearer ' + auth.getToken()}})
        .then(resp => {
          if (response.data.length === 0) {
            this.errors = 'Nenhum resultado encontrado'
            this.facility = ''
          } else {
            this.facility = resp.data
          }
        })
        .catch(error => {
          this.errors = error.data
          this.facility = ''
        })
      }
    })
    .catch(error => {
      this.errors = error.data
      this.equipment = ''
    })

    axios.get(auth.apiUrl() + 'equipment/GamaCamaraAsymmetricWindowTests/' + this.$route.params.testId, {headers: {Authorization: 'Bearer ' + auth.getToken()}})
    .then(response => {
      if (response.data.length === 0) {
        this.errors = 'Nenhum resultado encontrado'
        this.test = ''
      } else {
        this.test = response.data
      }
    })
    .catch(error => {
      this.errors = error.data
      this.test = ''
    })

    this.currentUser = auth.currentUser()
  },

  methods: {
    exclude () {
      if (confirm('Tem certeza que deseja excluir este teste?')) {
        auth.excludeGamaCamaraAsymmetricWindowTest(this, this.test, '/equipments/gamaCamara/' + this.equipment._id + '/asymmetricWindow')
      }
    },
    finishTest () {
      if (confirm('Tem certeza que deseja finalizar este teste?')) {
        this.test.status = 'Concluído'
        this.test.responsibleForFinalizing = { name: auth.currentUser().name, _id: auth.currentUser().id }
        var now = new Date()
        now = new Date(moment.tz(now, 'America/Sao_Paulo'))
        now = now.getFullYear() + '-' + ('0' + (now.getMonth() + 1)).slice(-2) + '-' + ('0' + now.getDate()).slice(-2)
        this.test.finalizingDate = now
        auth.updateGamaCamaraAsymmetricWindowTest(this, this.test)

        this.generateDocument()
        this.loading = true
        setTimeout(function () {
          router.push({ path: '/document' })
        }, 6000)
      }
    },

    addLabelAndInformation (doc, x, y, label, information) {
      var spaceBetweenInformationAndLabel = 5

      doc.setFontSize(8)
      doc.setFontStyle('bold')
      doc.text(label, x, y)
      doc.setFontSize(10)
      doc.setFontStyle('normal')
      if (information) {
        doc.text(information, x, y + spaceBetweenInformationAndLabel)
      } else {
        doc.text('', x, y + spaceBetweenInformationAndLabel)
      }
    },

    generateDocument () {
      var doc = new JsPDF()

      var radLogo = `data:image/png;base64,`
      var lineHeight = 12
      var tableLineHeight = 4
      var beginRoundedRect = 35
      var beginTextHeight = beginRoundedRect + 10

      var beginSecondRoundedRect = beginRoundedRect + 42 + 3
      var beginSecondTextHeight = beginSecondRoundedRect + 10

      var beginThirdRoundedRect = beginSecondRoundedRect + 9 + 2 * 4 + 3
      var beginThirdTextHeight = beginThirdRoundedRect + 10

      var beginFourthRoundedRect = beginThirdRoundedRect + 16 + 3
      var beginFourthTextHeight = beginFourthRoundedRect + 10

      var begintFifthRoudedRect = beginFourthRoundedRect + 59 + 3
      var begintFifthTextHeight = begintFifthRoudedRect + 10

      doc.roundedRect(10, 10, 190, 22, 2, 2, 'F')
      doc.addImage(radLogo, 'PNG', 15, 12, 74, 20)

      doc.roundedRect(10, beginRoundedRect, 190, 42, 2, 2, 'S')
      doc.setFontSize(12)
      doc.setFontStyle('bold')
      if (this.test.isAcceptanceTest) {
        doc.text(105, beginRoundedRect + 5, 'Controle de Qualidade - Teste de Aceite', null, null, 'center')
      } else {
        doc.text(105, beginRoundedRect + 5, 'Controle de Qualidade', null, null, 'center')
      }

      this.addLabelAndInformation(doc, 15, beginTextHeight, 'Instalação:', this.facility.companyName)
      this.addLabelAndInformation(doc, 150, beginTextHeight, 'Responsável pelo Teste:', this.test.responsibleForFinalizing.name)
      // this.addLabelAndInformation(doc, 90, beginTextHeight, 'Local:', this.equipment.localization)
      this.addLabelAndInformation(doc, 15, beginTextHeight + lineHeight, 'Equipamento:', this.equipment.type)
      this.addLabelAndInformation(doc, 83, beginTextHeight + lineHeight, 'Marca/Modelo/Número de Série:', this.equipment.manufacturer + '/' + this.equipment.model + '/' + this.equipment.serialNumber)
      this.addLabelAndInformation(doc, 150, beginTextHeight + lineHeight, 'Número Patrimonial:', this.equipment.patrimonialNumber)

      this.addLabelAndInformation(doc, 15, beginTextHeight + 2 * lineHeight, 'Teste:', 'Janelas Assimétricas')
      this.addLabelAndInformation(doc, 83, beginTextHeight + 2 * lineHeight, 'Periodicidade:', 'Anual')
      this.addLabelAndInformation(doc, 150, beginTextHeight + 2 * lineHeight, 'Data:', moment(this.test.date).format('DD/MM/YYYY'))

      doc.setFontSize(12)
      doc.setFontStyle('bold')
      doc.text(105, beginSecondRoundedRect + 5, 'Objetivos', null, null, 'center')

      doc.setFontSize(10)
      doc.setFontStyle('normal')
      doc.text(15, beginSecondTextHeight, 'Testar a resposta intrínseca de um ou mais detectores de cintilação para diferentes janelas energéticas, em busca')
      doc.text(15, beginSecondTextHeight + 4, 'de distorções que possam indicar alterações na qualidade das imagens, utilizando uma fonte de Tc-99m.')
      doc.roundedRect(10, beginSecondRoundedRect, 190, 9 + 2 * 4, 2, 2, 'S')

      doc.setFontSize(12)
      doc.setFontStyle('bold')
      doc.text(105, beginThirdRoundedRect + 5, 'Informações', null, null, 'center')

      doc.setFontSize(8)
      doc.setFontStyle('bold')
      doc.text(15, beginThirdTextHeight, 'Matrix Utilizada')
      doc.text(45, beginThirdTextHeight, 'Número de Contagens')
      doc.text(84, beginThirdTextHeight, 'Atividade (µCi)')
      doc.text(112, beginThirdTextHeight, 'Taxa de Contagem (Kcount/s)')
      doc.text(162, beginThirdTextHeight, 'Correção de Atenuação')
      doc.setFontSize(10)
      doc.setFontStyle('normal')
      doc.text(15, beginThirdTextHeight + 4, this.test.usedMatrix)
      doc.text(45, beginThirdTextHeight + 4, this.test.totalCountNumber)
      doc.text(84, beginThirdTextHeight + 4, this.test.pontualSourceActivity)
      doc.text(112, beginThirdTextHeight + 4, this.test.countTax)
      if (this.test.uniformityCorrection) {
        doc.text(162, beginThirdTextHeight + 4, 'Com correção')
      } else {
        doc.text(162, beginThirdTextHeight + 4, 'Sem correção')
      }

      doc.roundedRect(10, beginThirdRoundedRect, 190, 16, 2, 2, 'S')

      doc.setFontSize(12)
      doc.setFontStyle('bold')
      doc.text(105, beginFourthRoundedRect + 5, 'Análises', null, null, 'center')

      if (this.test.testedDetectors === 'Detector 1' || this.test.testedDetectors === 'Detector 1 e Detector 2') {
        doc.setFontSize(10)
        doc.setFontStyle('bold')
        doc.text(15, beginFourthTextHeight, 'Detector 1')
        doc.setFontSize(8)
        doc.text(15, beginFourthTextHeight + 4, 'Energia')
        doc.text(45, beginFourthTextHeight + 4, 'Unif. Integral')
        doc.text(68, beginFourthTextHeight + 4, 'Comentário')
        doc.setFontSize(9)
        doc.setFontStyle('normal')
        for (let i = 0; i < this.test.energyAndResultsDetector1.length; i++) {
          let datacheck = this.test.energyAndResultsDetector1[i].dataCheckStatus.split('(insatisfatório)')[0].split('(satisfatório)')[0]

          doc.text(15, beginFourthTextHeight + 8 + tableLineHeight * i, this.test.energyAndResultsDetector1[i].energy)
          doc.text(45, beginFourthTextHeight + 8 + tableLineHeight * i, parseFloat(this.test.energyAndResultsDetector1[i].integralCalculatedUniformity).toFixed(2) + '%')
          doc.text(68, beginFourthTextHeight + 8 + tableLineHeight * i, datacheck)
        }
        // doc.text(15, beginFourthTextHeight + 12 + tableLineHeight * this.test.energyAndResultsDetector1.length, this.test.energyAndResultsDetector1Obs)
      }

      if (this.test.testedDetectors === 'Detector 2' || this.test.testedDetectors === 'Detector 1 e Detector 2') {
        doc.setFontSize(10)
        doc.setFontStyle('bold')
        doc.text(15, 26 + beginFourthTextHeight, 'Detector 2')
        doc.setFontSize(8)
        doc.text(15, 26 + beginFourthTextHeight + 4, 'Energia')
        doc.text(45, 26 + beginFourthTextHeight + 4, 'Unif. Integral')
        doc.text(68, 26 + beginFourthTextHeight + 4, 'Comentário')
        doc.setFontSize(9)
        doc.setFontStyle('normal')
        for (let i = 0; i < this.test.energyAndResultsDetector2.length; i++) {
          let datacheck = this.test.energyAndResultsDetector2[i].dataCheckStatus.split('(insatisfatório)')[0].split('(satisfatório)')[0]

          doc.text(15, 26 + beginFourthTextHeight + 8 + tableLineHeight * i, this.test.energyAndResultsDetector2[i].energy)
          doc.text(45, 26 + beginFourthTextHeight + 8 + tableLineHeight * i, parseFloat(this.test.energyAndResultsDetector2[i].integralCalculatedUniformity).toFixed(2) + '%')
          doc.text(68, 26 + beginFourthTextHeight + 8 + tableLineHeight * i, datacheck)
        }
      }

      doc.roundedRect(10, beginFourthRoundedRect, 190, 59, 2, 2, 'S')

      doc.setFontSize(12)
      doc.setFontStyle('bold')
      doc.text(105, begintFifthRoudedRect + 5, 'Resultados', null, null, 'center')

      if (this.test.testedDetectors === 'Detector 1' || this.test.testedDetectors === 'Detector 1 e Detector 2') {
        doc.setFontSize(10)
        doc.setFontStyle('bold')
        doc.text(15, begintFifthTextHeight, 'Detector 1')
        doc.setFontSize(9)
        doc.setFontStyle('normal')
        if (this.test.results[0] === 'As imagens do detector não são uniformes na região de energia utilizada durante exames diagnósticos. O detector não é considerado apto para uso e o fabricante deve ser contatado para manutenção, verificação de desajustes em fotomutiplicadoras e necessidade de refazer o mapa de correção.') {
          doc.text(15, begintFifthTextHeight + 4, 'As imagens do detector não são uniformes na região de energia utilizada durante exames diagnósticos. O detector não é')
          doc.text(15, begintFifthTextHeight + 4 + 4, 'considerado apto para uso e o fabricante deve ser contatado para manutenção, verificação de desajustes em fotomutiplicadoras')
          doc.text(15, begintFifthTextHeight + 4 + 8, 'e necessidade de refazer o mapa de correção.')
        } else if (this.test.results[0] === 'As imagens do detector são uniformes na região de energia utilizada durante exames diagnósticos. O detector é considerado apto para uso porém o fabricante deve ser contatado para verificação do de desajuste em fotomultiplicadoras e necessidade de refazer o mapa de correção.') {
          doc.text(15, begintFifthTextHeight + 4, 'As imagens do detector são uniformes na região de energia utilizada durante exames diagnósticos. O detector é')
          doc.text(15, begintFifthTextHeight + 4 + 4, 'considerado apto para uso porém o fabricante deve ser contatado para verificação do de desajuste em fotomultiplicadoras')
          doc.text(15, begintFifthTextHeight + 4 + 8, 'e necessidade de refazer o mapa de correção.')
        } else {
          doc.text(15, begintFifthTextHeight + 4, 'As imagens do detector são uniformes, não apresentando pontos de hiper ou hipo-captação, nem regiões de alto contraste')
          doc.text(15, begintFifthTextHeight + 4 + 4, 'frio ou quente. O detector é considerado apto para uso.')
        }
        doc.setFontStyle('bold')
        doc.text(15, begintFifthTextHeight + 5 + 12, 'Observações:')
        doc.setFontStyle('normal')
        doc.text(15 + 22, begintFifthTextHeight + 5 + 12, this.test.energyAndResultsDetector1Obs)
        doc.setFontStyle('bold')
        doc.text(15, begintFifthTextHeight + 5 + 16, 'Conclusão:')
        doc.setFontStyle('normal')
        doc.text(15 + 18, begintFifthTextHeight + 5 + 16, this.test.conclusions[0])
      }

      if (this.test.testedDetectors === 'Detector 2' || this.test.testedDetectors === 'Detector 1 e Detector 2') {
        doc.setFontSize(10)
        doc.setFontStyle('bold')
        doc.text(15, begintFifthTextHeight + 28, 'Detector 2')
        doc.setFontSize(9)
        doc.setFontStyle('normal')
        if (this.test.results[1] === 'As imagens do detector não são uniformes na região de energia utilizada durante exames diagnósticos. O detector não é considerado apto para uso e o fabricante deve ser contatado para manutenção, verificação de desajustes em fotomutiplicadoras e necessidade de refazer o mapa de correção.') {
          doc.text(15, begintFifthTextHeight + 28 + 4, 'As imagens do detector não são uniformes na região de energia utilizada durante exames diagnósticos. O detector não é')
          doc.text(15, begintFifthTextHeight + 28 + 4 + 4, 'considerado apto para uso e o fabricante deve ser contatado para manutenção, verificação de desajustes em fotomutiplicadoras')
          doc.text(15, begintFifthTextHeight + 28 + 4 + 8, 'e necessidade de refazer o mapa de correção.')
        } else if (this.test.results[1] === 'As imagens do detector são uniformes na região de energia utilizada durante exames diagnósticos. O detector é considerado apto para uso porém o fabricante deve ser contatado para verificação do de desajuste em fotomultiplicadoras e necessidade de refazer o mapa de correção.') {
          doc.text(15, begintFifthTextHeight + 28 + 4, 'As imagens do detector são uniformes na região de energia utilizada durante exames diagnósticos. O detector é considerado')
          doc.text(15, begintFifthTextHeight + 28 + 4 + 4, 'apto para uso porém o fabricante deve ser contatado para verificação do de desajuste em fotomultiplicadoras e necessidade')
          doc.text(15, begintFifthTextHeight + 28 + 4 + 8, 'de refazer o mapa de correção.')
        } else {
          doc.text(15, begintFifthTextHeight + 28 + 4, 'As imagens do detector são uniformes, não apresentando pontos de hiper ou hipo-captação, nem regiões de alto contraste')
          doc.text(15, begintFifthTextHeight + 28 + 4 + 4, 'frio ou quente. O detector é considerado apto para uso.')
        }
        doc.setFontStyle('bold')
        doc.text(15, begintFifthTextHeight + 28 + 5 + 12, 'Observações:')
        doc.setFontStyle('normal')
        doc.text(15 + 22, begintFifthTextHeight + 28 + 5 + 12, this.test.energyAndResultsDetector2Obs)
        doc.setFontStyle('bold')
        doc.text(15, begintFifthTextHeight + 28 + 5 + 16, 'Conclusão:')
        doc.setFontStyle('normal')
        doc.text(15 + 18, begintFifthTextHeight + 28 + 5 + 16, this.test.conclusions[1])
      }

      doc.roundedRect(10, begintFifthRoudedRect, 190, 62, 2, 2, 'S')

      doc.setFontSize(12)
      doc.setFontStyle('bold')
      doc.text(105, begintFifthRoudedRect + 62 + 5 + 3, 'Bibliografia', null, null, 'center')
      doc.setFontSize(8)
      doc.setFontStyle('normal')
      doc.text(15, begintFifthRoudedRect + 62 + 10 + 3, '[1] INTERNATIONAL ATOMIC ENERGY AGENCY. IAEA-HUMAN HEALTH SERIES Nº 6: Quality assurance for SPECT systems. Vienna. 2009')
      doc.text(15, begintFifthRoudedRect + 62 + 10 + 3 + 4, '[2] INTERNATIONAL ATOMIC ENERGY AGENCY. IAEA- Quality control atlas for scintillation camera systems. Vienna. 2003')

      doc.roundedRect(10, begintFifthRoudedRect + 62 + 3, 190, 17, 2, 2, 'S')

      doc.setFontSize(10)
      doc.setFontStyle('normal')
      doc.text(190, 280, '1/1')

      doc.save('Informações ' + this.facility.name + '.pdf')
    }
  }
}
</script>

<style scoped>
[data-letters-list]:before {
  content:attr(data-letters-list);
  display:inline-block;
  font-size:1em;
  width:3em;
  height:3em;
  line-height:3em;
  text-align:center;
  border-radius:50%;
  background: #00a767;
  vertical-align:middle;
  margin-right:1em;
  color:white;
}

.fa {
  width: 14px;
  height: 14px;
}
</style>
../../../../../../../../services/permissions