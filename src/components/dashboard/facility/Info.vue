<template>
  <div class="main">
    <div class="col-12 row px-0 mx-0">
      <div class="col">
        <h1>{{ facility.name }}</h1>
      </div>
      <div class="col text-right">
        <a href="#" @click="downloadFacilityDocument" class="btn btn-outline-metalic-seaweed"><i class="fa fa-download d-block d-md-none export-button" aria-hidden="true"></i><span class="d-none d-md-block"><i class="fa fa-download" aria-hidden="true"></i> exportar</span></a>
      </div>
    </div>
    <div class="col py-0">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb my-0">
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/panel'"><i class="fa fa-home" aria-hidden="true"></i></router-link></li>
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/facility'"><i class="far fa-building" aria-hidden="true"></i></router-link></li>
          <li class="breadcrumb-item" aria-current="page"><router-link :to="'/facility/' + facility._id">{{ facility.name }}</router-link></li>
          <li class="breadcrumb-item active" aria-current="page">informações</li>
        </ol>
      </nav>
    </div>

    <div class="row ml-md-3">
      <div class="col mb-3 pl-sm-1">
        <div class="hyper-card mb-3">
          <h5 class="mb-3">Informações:</h5>
          <p class="pl-3" v-if="facility.companyName"><i class="far fa-building" aria-hidden="true"></i><strong class="px-1">Nome:</strong> {{ facility.companyName }}</p>
          <p class="pl-3"><i class="far fa-id-card" aria-hidden="true"></i><strong class="px-1">CNPJ:</strong> {{ facility.cnpj }}</p>
          <p class="pl-3" v-if="facility.telephone"><i class="fa fa-phone" aria-hidden="true"></i><strong class="px-1">Telefone:</strong> {{ facility.telephone }}</p>
          <p class="pl-3" v-if="facility.contactName"><i class="fa fa-user" aria-hidden="true"></i><strong class="px-1">Contato:</strong> {{ facility.contactName }}</p>
          <p class="pl-3" v-if="facility.CNENregistry"><i class="far fa-file" aria-hidden="true"></i><strong class="px-1">Registro CNEN:</strong> {{ facility.CNENregistry }}</p>
        </div>
        <div v-if="facility.radioisotopes" class="hyper-card">
          <h5 class="mb-3">Quantidade de radionuclídeos autorizados por semana:</h5>
          <template v-for="radioisotope in facility.radioisotopes">
            <p class="pl-3" v-if="radioisotope.quantity && radioisotope.quantity > 0"><b>{{ radioisotope.name }}:</b> {{ radioisotope.quantity }} mCi</p>
          </template>
        </div>
      </div>
      <div class="col mb-3">
        <div v-if="facility.address" class="hyper-card mb-3">
          <h5 class="mb-3">Endereço:</h5>
          <p class="pl-3"><i class="fa fa-map-marker" aria-hidden="true"></i><strong class="px-1"></strong> {{ facility.address[0].street }}</p>
        </div>
        <div v-if="facility.doDiagnosis || facility.doInpatientTherapy || facility.doOutpatientTherapy" class="hyper-card">
          <h5 class="mb-3">Práticas autorizadas:</h5>
          <p class="pl-3 mb-0" v-if="facility.doDiagnosis" ><i class="fa fa-check" aria-hidden="true"></i> Diagnóstico</p>
          <p class="pl-3 mb-0" v-if="facility.doInpatientTherapy"><i class="fa fa-check" aria-hidden="true"></i> Terapia com Internação</p>
          <p class="pl-3 mb-0" v-if="facility.doOutpatientTherapy"><i class="fa fa-check" aria-hidden="true"></i> Terapia Ambulatorial</p>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import auth from '../../authentication/authentication'
import permission from '../../../../services/permissions'
import axios from 'axios'
import JsPDF from 'jspdf'

export default {

  data () {
    return {
      facility: {
        users: [],
        equipments: [],
        radioisotopes: []
      },
      dataLetters: '',
      currentUser: {},
      permission: permission,
      columns: [
        {
          label: ''
        },
        {
          label: 'Nome',
          field: 'name'
        },
        {
          label: 'Responsabilidade',
          field: 'responsability'
        }
      ]
    }
  },

  created () {
    axios.get(auth.apiUrl() + 'facility/' + this.$route.params.id, {headers: {Authorization: 'Bearer ' + auth.getToken()}})
    .then(response => {
      if (response.data.length === 0) {
        this.errors = 'Nenhum resultado encontrado'
        this.facility = ''
      } else {
        this.facility._id = response.data._id
        this.facility.name = response.data.name
        this.facility.companyName = response.data.companyName
        this.facility.cnpj = response.data.cnpj
        this.facility.telephone = response.data.telephone
        this.facility.contactName = response.data.contactName
        this.facility.address = response.data.address

        this.facility.CNENregistry = response.data.CNENregistry
        this.facility.professionCNEN = response.data.professionCNEN
        this.facility.professionNumberCNEN = response.data.professionNumberCNEN
        this.facility.validityOfOperationAuthorization = response.data.validityOfOperationAuthorization
        this.facility.doInpatientTherapy = response.data.doInpatientTherapy
        this.facility.doOutpatientTherapy = response.data.doOutpatientTherapy
        this.facility.doDiagnosis = response.data.doDiagnosis
        this.facility.radioisotopes = response.data.radioisotopes

        this.facility.users = response.data.users
      }
    })
    .catch(error => {
      this.errors = error.data
      this.facility = ''
    })
    this.currentUser = auth.currentUser()
  },

  methods: {
    exclude () {
      if (confirm('Tem certeza que deseja excluir esta instalação?')) {
        auth.excludeFacility(this, this.facility, '/facilities')
      }
    },
    downloadFacilityProfession () {
      var professionCNEN = document.getElementById('professionCNEN')
      professionCNEN.href = this.facility.professionCNEN

      professionCNEN.click()
    },

    addLabelAndInformation (doc, x, y, label, information) {
      var spaceBetweenInformationAndLabel = 5

      doc.setFontSize(8)
      doc.setFontStyle('bold')
      doc.text(label, x, y)
      doc.setFontSize(10)
      doc.setFontStyle('normal')
      if (information) {
        doc.text(information, x, y + spaceBetweenInformationAndLabel)
      } else {
        doc.text('', x, y + spaceBetweenInformationAndLabel)
      }
    },

    downloadFacilityDocument () {
      var doc = new JsPDF()

      var radLogo = `data:image/png;base64,`
      var lineHeight = 12
      var beginRoundedRect = 35
      var beginTextHeight = beginRoundedRect + 15

      var beginSecondRoundedRect = beginRoundedRect + 75 + 5
      var beginSecondTextHeight = beginSecondRoundedRect + 15

      var beginThirdRoundedRect = beginSecondRoundedRect + 75 + 5
      var beginThirdTextHeight = beginThirdRoundedRect + 15

      doc.roundedRect(10, 10, 190, 22, 2, 2, 'F')
      doc.addImage(radLogo, 'PNG', 15, 12, 74, 20)

      doc.roundedRect(10, beginRoundedRect, 190, 75, 2, 2, 'S')
      doc.setFontSize(12)
      doc.setFontStyle('bold')
      doc.text(105, beginRoundedRect + 8, 'Infromações da Instalação', null, null, 'center')

      this.addLabelAndInformation(doc, 15, beginTextHeight, 'Razão Social:', this.facility.companyName)
      this.addLabelAndInformation(doc, 15, beginTextHeight + lineHeight, 'CNPJ:', this.facility.cnpj)
      this.addLabelAndInformation(doc, 90, beginTextHeight + lineHeight, 'Matrícula CNEN:', this.facility.CNENregistry)
      this.addLabelAndInformation(doc, 150, beginTextHeight + lineHeight, 'Telefone:', this.facility.telephone)
      this.addLabelAndInformation(doc, 15, beginTextHeight + 2 * lineHeight, 'Endereço:', this.facility.address[0].street)
      this.addLabelAndInformation(doc, 15, beginTextHeight + 3 * lineHeight, 'Número:', this.facility.address[0].number)
      this.addLabelAndInformation(doc, 90, beginTextHeight + 3 * lineHeight, 'Complemento:', this.facility.address[0].complement)
      this.addLabelAndInformation(doc, 150, beginTextHeight + 3 * lineHeight, 'Bairro:', this.facility.address[0].neighborhood)
      this.addLabelAndInformation(doc, 15, beginTextHeight + 4 * lineHeight, 'Cidade:', this.facility.address[0].city)
      this.addLabelAndInformation(doc, 90, beginTextHeight + 4 * lineHeight, 'Estado:', this.facility.address[0].state)

      doc.roundedRect(10, beginSecondRoundedRect, 190, 75, 2, 2, 'S')
      doc.setFontSize(12)
      doc.setFontStyle('bold')
      doc.text(105, beginSecondRoundedRect + 8, 'Responsáveis pela Instalação', null, null, 'center')
      if (this.facility.users.filter(user => user.responsabilities.indexOf('Titular da instalação') >= 0).length > 0) {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight, 'Titular da Instalação:', this.facility.users.filter(user => user.responsabilities.indexOf('Titular da instalação') >= 0)[0].name)
      } else {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight, 'Titular da Instalação:', '')
      }
      if (this.facility.users.filter(user => user.responsabilities.indexOf('Responsável técnico') >= 0).length > 0) {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight + lineHeight, 'Responsável Técnico:', this.facility.users.filter(user => user.responsabilities.indexOf('Responsável técnico') >= 0)[0].name)
        this.addLabelAndInformation(doc, 150, beginSecondTextHeight + lineHeight, 'Licença AN:', this.facility.users.filter(user => user.responsabilities.indexOf('Responsável técnico') >= 0)[0].ANLicenseNumber)
      } else {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight + lineHeight, 'Responsável Técnico:', '')
        this.addLabelAndInformation(doc, 150, beginSecondTextHeight + lineHeight, 'Licença AN:', '')
      }
      if (this.facility.users.filter(user => user.responsabilities.indexOf('Substituto do responsável técnico') >= 0).length > 0) {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight + 2 * lineHeight, 'Substituto do Responsável Técnico:', this.facility.users.filter(user => user.responsabilities.indexOf('Substituto do responsável técnico') >= 0)[0].name)
        this.addLabelAndInformation(doc, 150, beginSecondTextHeight + 2 * lineHeight, 'Licença AN:', this.facility.users.filter(user => user.responsabilities.indexOf('Substituto do responsável técnico') >= 0)[0].ANLicenseNumber)
      } else {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight + 2 * lineHeight, 'Substituto do Responsável Técnico:', '')
        this.addLabelAndInformation(doc, 150, beginSecondTextHeight + 2 * lineHeight, 'Licença AN:', '')
      }
      if (this.facility.users.filter(user => user.responsabilities.indexOf('Supervisor de radioproteção') >= 0).length > 0) {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight + 3 * lineHeight, 'Supervisor de Radioproteção:', this.facility.users.filter(user => user.responsabilities.indexOf('Supervisor de radioproteção') >= 0)[0].name)
        this.addLabelAndInformation(doc, 150, beginSecondTextHeight + 3 * lineHeight, 'Licença FM:', this.facility.users.filter(user => user.responsabilities.indexOf('Supervisor de radioproteção') >= 0)[0].FMLicenseNumber)
      } else {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight + 3 * lineHeight, 'Supervisor de Radioproteção:', '')
        this.addLabelAndInformation(doc, 150, beginSecondTextHeight + 3 * lineHeight, 'Licença FM:', '')
      }
      if (this.facility.users.filter(user => user.responsabilities.indexOf('Substituto do supervisor de radioproteção') >= 0).length > 0) {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight + 4 * lineHeight, 'Substituto do Supervisor de Radioproteção:', this.facility.users.filter(user => user.responsabilities.indexOf('Substituto do supervisor de radioproteção') >= 0)[0].name)
        this.addLabelAndInformation(doc, 150, beginSecondTextHeight + 4 * lineHeight, 'Licença FM:', this.facility.users.filter(user => user.responsabilities.indexOf('Substituto do supervisor de radioproteção') >= 0)[0].FMLicenseNumber)
      } else {
        this.addLabelAndInformation(doc, 15, beginSecondTextHeight + 4 * lineHeight, 'Substituto do Supervisor de Radioproteção:', '')
        this.addLabelAndInformation(doc, 150, beginSecondTextHeight + 4 * lineHeight, 'Licença FM:', '')
      }

      doc.roundedRect(10, beginThirdRoundedRect, 190, 75, 2, 2, 'S')
      doc.setFontSize(12)
      doc.setFontStyle('bold')
      doc.text(105, beginThirdRoundedRect + 8, 'Práticas e Radioisótopos Autorizados', null, null, 'center')
      var authorizedPractices = ''
      if (this.facility.doInpatientTherapy) {
        authorizedPractices += 'Terapia com internação; '
      }
      if (this.facility.doOutpatientTherapy) {
        authorizedPractices += 'Terapia ambulatorial; '
      }
      if (this.facility.doDiagnosis) {
        authorizedPractices += 'Diagnóstico; '
      }
      doc.setFontSize(10)
      doc.setFontStyle('normal')
      this.addLabelAndInformation(doc, 15, beginThirdTextHeight, 'Práticas autorizadas:', authorizedPractices)

      var columnIndex = 1
      var lineIndex = 1
      for (var i = 0; i < this.facility.radioisotopes.length; i++) {
        if (this.facility.radioisotopes[i].quantity > 0) {
          if (Math.floor(i / 4) === i / 4) {
            columnIndex = columnIndex + 1
            lineIndex = 1
          }
          lineIndex = lineIndex + 1

          this.addLabelAndInformation(doc, (columnIndex - 1) * 30 - 15, beginThirdTextHeight + (lineIndex - 1) * lineHeight, this.facility.radioisotopes[i].name + ': ', this.facility.radioisotopes[i].quantity + ' mCi')
        }
      }

      doc.save('Informações ' + this.facility.name + '.pdf')
    }
  }
}
</script>

<style scoped>
[data-letters-list]:before {
  content:attr(data-letters-list);
  display:inline-block;
  font-size:1em;
  width:3em;
  height:3em;
  line-height:3em;
  text-align:center;
  border-radius:50%;
  background: #00a767;
  vertical-align:middle;
  margin-right:1em;
  color:white;
}

.export-button {
  line-height: 24px;
  width: 14px;
  height: 24px;
}
</style>
../../../../services/permissions