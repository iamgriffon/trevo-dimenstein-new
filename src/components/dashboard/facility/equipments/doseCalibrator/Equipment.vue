<template>
  <div class="main">
    ...
    <div class="col-12 row px-0 mx-0">
      <router-link :to="'/equipments/doseCalibrator/' + equipment._id + '/linearity/'" class="hyper-card col m-3 left-border-eagle-green hyper-card-shadow-hover">
        <h3>Linearidade</h3>
      </router-link>
      <router-link :to="'/equipments/doseCalibrator/' + equipment._id + '/accuracyandprecision/'"  class="hyper-card col m-3 left-border-metalic-seaweed hyper-card-shadow-hover">
        <h3>Exatidão e Precisão</h3>
      </router-link>
      <router-link :to="'/equipments/doseCalibrator/' + equipment._id + '/daily/' + dateYear" class="hyper-card col m-3 left-border-queen-blue hyper-card-shadow-hover">
        <h3>Diário</h3>
      </router-link>
    </div>
    <div class="col-12 row px-0 mx-0">
      <router-link :to="'/equipments/doseCalibrator/' + equipment._id + '/geometry/'" class="hyper-card col m-3 left-border-queen-blue hyper-card-shadow-hover">
        <h3>Geometria</h3>
      </router-link>
    </div>
    ...
    <div class="row ml-md-3">
      <div class="col mb-3 pl-sm-1">
        <div class="hyper-card mb-3">
          ...
          <p class="pl-3" v-if="equipment.mostUsedRadioisotope"><i class="fab fa-react" aria-hidden="true"></i> Radioisótopo mais utilizado: {{ equipment.mostUsedRadioisotope }}</p>
        </div>
      </div>
    </div>
    <br>
    <div class="col-12 row m-0 p-0">
      <div class="col pb-2">
        <h3>Dados para Teste Diário</h3>
      </div>
      <div class="col text-right">
        <router-link v-if="permission.canCreateFacility(currentUser)" class="btn btn-outline-success" :to="{ path: '/equipments/doseCalibrator/' + $route.params.id + '/source/register'}"><i class="fa fa-plus d-sm-block d-md-none" aria-hidden="true"></i><span class="d-none d-sm-block"><i class="fa fa-plus" aria-hidden="true"></i> adicionar</span></router-link>
      </div>
    </div>
    <div class="row ml-md-3">
      <div class="col-12 col-md-4 mb-3 pl-sm-1" v-for="source in sources">
        <div class="hyper-card mb-3">
          <div class="row">
            ...
            <div class="col text-right">
              <router-link :to="'/equipments/doseCalibrator/' + $route.params.id + '/source/' + source._id + '/edit'" v-if="permission.canEditFacility(currentUser)" class="btn btn-outline-metalic-seaweed"><i class="fas fa-edit d-sm-block d-md-none" aria-hidden="true"></i><span class="d-none d-md-block"><i class="fas fa-edit" aria-hidden="true"></i></span></router-link>
              <button @click="excludeSource(source._id)" v-if="permission.canExcludeFacility(currentUser)" class="btn btn-outline-danger"><i class="far fa-trash-alt d-sm-block d-md-none" aria-hidden="true"></i><span class="d-none d-md-block"><i class="far fa-trash-alt" aria-hidden="true"></i></span></button>
            </div>
          </div>
