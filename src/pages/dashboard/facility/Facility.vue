<template>
  <div class="main">
    <div class="col-12 row m-0 p-0">
      ...
      <div class="col text-right">
        <router-link
          ...
          class="btn btn-outline-metalic-seaweed"
          ><i class="fa fa-info d-sm-block d-md-none" aria-hidden="true"></i
          ><span class="d-none d-md-block"
            ><i class="fa fa-info" aria-hidden="true"></i> informações</span
          ></router-link
        >
        <router-link
          ...
          ><i class="fas fa-edit d-sm-block d-md-none" aria-hidden="true"></i
          ><span class="d-none d-md-block"
            ><i class="fas fa-edit" aria-hidden="true"></i> editar</span
          ></router-link
        >
        <button
          @click="exclude()"
        ...
        >
          <i
            class="far fa-trash-alt d-sm-block d-md-none"
            aria-hidden="true"
          ></i
          ><span class="d-none d-md-block"
            ><i class="far fa-trash-alt" aria-hidden="true"></i> excluir</span
          >
        </button>
      </div>
    </div>
    ...
    <div class="col-12 row px-0 mx-0">
      <router-link
        :to="'/facility/' + facility._id + '/documents'"
        class="hyper-card col m-3 left-border-eagle-green hyper-card-shadow-hover"
      >
        <h3>Lista de Documentos</h3>
      </router-link>
      <router-link
        :to="'/facility/' + facility._id + '/equipments'"
        class="hyper-card col m-3 left-border-metalic-seaweed hyper-card-shadow-hover"
      >
        <h3>Equipamentos</h3>
      </router-link>
      <router-link
        :to="'/facility/' + facility._id + '/routines'"
        class="hyper-card col m-3 left-border-queen-blue hyper-card-shadow-hover"
      >
        <h3>Rotinas</h3>
      </router-link>
    </div>
    ...
    <div class="col-12">
      ...
      <vue-good-table
        ...
        :rows="facility.users"
        :paginate="false"
        styleClass="table hyper-table pl-0 pl-md-2 ml-0 ml-md-2 mr-2 col-12"
      >
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'icon'">
            <span
              :data-letters-list="
                props.row.name.split(' ')[0].charAt(0) +
                props.row.name.split(' ')[1].charAt(0)
              "
            ></span>
          </span>
...
export default {
  ...
  created() {
    ...
    axios
      .get(auth.apiUrl() + "facility/" + this.$route.params.id, {
      ...
      .then((response) => {
        ...
        if (response.data.length === 0) {
          ...
          this.facility.professionNumberCNEN =
            response.data.professionNumberCNEN;
          this.facility.validityOfOperationAuthorization =
            response.data.validityOfOperationAuthorization;
  ...
  methods: {
    loadTypesOfEquipment() {
      ...
      axios
        .get(auth.apiUrl() + "typeOfEquipment", {
          headers: { Authorization: "Bearer " + auth.getToken() },
        })
        .then((response) => {
          if (response.data.length === 0) {
                      ...
                      .then((r) => {
                        if (!r.data.length) {
                          ...
                          for (const typeOfTest of r.data) {
                            if (
                              !this.typesOfTest.find((e) => {
                                return e._id === typeOfTest._id;
                              })
                            ) {
                              this.typesOfTest.push({
                                name:
                                  typeOfTest.typeOfEquipment +
                                  " - " +
                                  typeOfTest.name,
