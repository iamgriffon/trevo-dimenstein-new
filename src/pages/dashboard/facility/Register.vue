  ...
  <Layout title="Criar Nova Instalação">
    ...
    <div class="flex flex-col gap-6 w-full">
      <section class="bg-white p-6 rounded-lg shadow">
        <div class="mb-4">
          <h4 class="font-medium mb-2"><span class="text-red-500">*</span>Departamento:</h4>
          <select v-model="facility.department" class="w-full p-2 border border-gray-300 rounded-md">
      ...
      <section class="bg-white p-6 rounded-lg shadow">
        <div class="mb-4">
          ...
          <select v-model="facility.group" class="w-full p-2 border border-gray-300 rounded-md">
            <option v-for="option in groupOptions" :key="option.name._id" :value="option.name">
              {{ option.label }}
            </option>
          </select>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block mb-2" for="logoFile">
              ...
              <small class="text-gray-500 block">escolha uma imagem quadrada</small>
            </label>
            <input 
              type="file" 
              id="logoFile" 
      ...
      <section class="bg-white p-6 rounded-lg shadow">
        <div v-if="error" class="bg-red-100 border border-red-400 text-red-700 p-4 rounded mb-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label class="block mb-2" for="facilityName">
              <FontAwesomeIcon icon="fa-solid fa-building" /> Nome<span class="text-red-500">*</span>
            </label>
            <input 
              type="text" 
              id="facilityName" 
          ...
          <div>
            <label class="block mb-2" for="companyName">
              <FontAwesomeIcon icon="fa-solid fa-building" /> Razão Social<span class="text-red-500">*</span>
            </label>
            <input 
              type="text" 
              id="companyName" 
        ...
        <div class="mt-4">
          <label class="block mb-2" for="address">
            <FontAwesomeIcon icon="fa-solid fa-map-marker" /> Endereço<span class="text-red-500">*</span>
            <small class="text-gray-500 block">formato: Logradouro, número - Bairro - Cidade - UF CEP</small>
          </label>
          <input 
            type="text" 
            id="address" 
      ...
      <section class="bg-white p-6 rounded-lg shadow">
        ...
        <div class="flex flex-col gap-4 mb-6">
          <div class="flex flex-col sm:flex-row sm:items-center sm:gap-2">
            <label class="block mb-2 sm:mb-0 sm:min-w-[180px]">Titular da Instalação:</label>
            <button 
          ...
          <div class="flex flex-col sm:flex-row sm:items-center sm:gap-2">
            <label class="block mb-2 sm:mb-0 sm:min-w-[180px]">Responsável técnico:</label>
            <button 
              v-if="tecnicalReponsible" 
              class="bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"
              title="Para alterar o usuário, remova-o da lista de pessoas selecionadas abaixo"
            >
              <FontAwesomeIcon icon="fa-solid fa-user" /> {{ tecnicalReponsible }}
            </button>
            <button 
              v-else 
              class="border border-green-600 text-green-600 px-4 py-2 rounded-lg hover:bg-green-600 hover:text-white transition-colors"
          ...
          <div class="flex flex-col sm:flex-row sm:items-center sm:gap-2">
            <label class="block mb-2 sm:mb-0 sm:min-w-[180px]">Substituto do responsável técnico:</label>
            <button 
              v-if="tecnicalReponsibleSubs" 
              class="bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"
              title="Para alterar o usuário, remova-o da lista de pessoas selecionadas abaixo"
            >
              <FontAwesomeIcon icon="fa-solid fa-user" /> {{ tecnicalReponsibleSubs }}
            </button>
            <button 
              v-else 
              class="border border-green-600 text-green-600 px-4 py-2 rounded-lg hover:bg-green-600 hover:text-white transition-colors"
          ...
          <div class="flex flex-col sm:flex-row sm:items-center sm:gap-2">
            <label class="block mb-2 sm:mb-0 sm:min-w-[180px]">Supervisor de radioproteção:</label>
            <button 
              v-if="radioprotectionSupervisor" 
              class="bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"
              title="Para alterar o usuário, remova-o da lista de pessoas selecionadas abaixo"
            >
              <FontAwesomeIcon icon="fa-solid fa-user" /> {{ radioprotectionSupervisor }}
            </button>
            <button 
              v-else 
              class="border border-green-600 text-green-600 px-4 py-2 rounded-lg hover:bg-green-600 hover:text-white transition-colors"
          ...
          <div class="flex flex-col sm:flex-row sm:items-center sm:gap-2">
            <label class="block mb-2 sm:mb-0 sm:min-w-[180px]">Substituto do supervisor de radioproteção:</label>
            <button 
              v-if="radioprotectionSupervisorSub" 
              class="bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"
              title="Para alterar o usuário, remova-o da lista de pessoas selecionadas abaixo"
            >
              <FontAwesomeIcon icon="fa-solid fa-user" /> {{ radioprotectionSupervisorSub }}
            </button>
            <button 
              v-else 
              class="border border-green-600 text-green-600 px-4 py-2 rounded-lg hover:bg-green-600 hover:text-white transition-colors"
        ...
        <h5 class="text-lg font-medium mb-4">
          ...
          <small class="text-gray-500 block">opcional. Usuários rad dimenstein possuem acesso a todas as instalações por padrão.</small>
        </h5>
        ...
        <div v-if="!secondFilteredUsers.length" class="bg-white p-6 rounded-lg text-center">
        ...
        <div v-else class="mb-8">
          <Table 
            ...
            :totalItems="secondFilteredUsers.length"
            :currentPage="1"
            :perPage="10"
          />
        </div>
      ...
      <section class="bg-white p-6 rounded-lg shadow">
        ...
        <div class="mb-6">
          <h4 class="font-medium mb-2">Práticas Autorizadas: <small class="text-gray-500">opcional</small></h4>
          <div class="flex flex-wrap gap-4">
            <label class="flex items-center gap-2">
              <input type="checkbox" class="w-5 h-5" v-model="facility.doInpatientTherapy" />
              Terapia com Internação
            </label>
            <label class="flex items-center gap-2">
              <input type="checkbox" class="w-5 h-5" v-model="facility.doOutpatientTherapy" />
              Terapia Ambulatorial
            </label>
            <label class="flex items-center gap-2">
              <input type="checkbox" class="w-5 h-5" v-model="facility.doDiagnosis" />
              Diagnóstico
            </label>
          </div>
        </div>
        ...
        <div>
          <h4 class="font-medium mb-4">Quantidade de radionuclídeos autorizados por semana: <small class="text-gray-500">opcional</small></h4>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div v-for="radioisotope in facility.radioisotopes" :key="radioisotope.name" class="flex items-center gap-2">
              <input 
                type="checkbox" 
                class="w-5 h-5" 
                :checked="radioisotope.quantity > 0"
                @click="radioisotope.quantity = radioisotope.quantity == null ? 0 : null" 
              />
    ...
    >
      <div class="flex flex-col gap-4">
        ...
        <div v-if="!filteredUsers.length" class="bg-white p-6 rounded-lg text-center">
          <h4 class="text-lg font-medium">Nenhum resultado</h4>
        </div>
        ...
        <Table 
          ...
          :currentPage="1"
          :perPage="10"
          @row-click="(user) => { addToSelectedUsers(user._id); selectResponsability(user._id, 'Titular da instalação'); }"
        />
    ...
    <Modal
      ...
      :class="'justify-between p-8'"
    >
      <!-- Similar content to chooseTitular modal -->
    </Modal>
    
    <!-- Add the rest of the modals -->
  </Layout>
</template>
...
const columnsWithResponsability = [
  ...
  },
  {
    ...
    cell: (info) => info.row.original.responsabilities.join(", ")
  }
];
...
const filteredUsers = computed(() => {
  if (filter.value.length > 0) {
    ...
    return users.value.filter(user => exp.test(user.name));
  }
  return users.value;
});
...
const secondFilteredUsers = computed(() => {
  if (secondFilter.value.length > 0) {
    ...
    return filtered.filter(user => !facility.value.users.some(u => u._id === user._id));
  }
  return users.value.filter(user => !facility.value.users.some(u => u._id === user._id));
});
...
function selectResponsability(userId, userResponsability) {
...
function removeFromSelectedUsers(userId) {
  resetFilters();
  const userIndex = facility.value.users.findIndex(user => user._id === userId);
...
function register() {
  const credentials = {
    ...
    validityOfOperationAuthorization: facility.value.validityOfOperationAuthorization,
...
onMounted(async () => {
  try {
    ...
    if (groupsResponse.data.length > 0) {
      groupOptions.value = groupsResponse.data.map(group => ({
